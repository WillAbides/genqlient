...
// TestOmitImplsCatchAllNameCollision verifies that
// omit_unreferenced_implementations gracefully handles schemas containing a
// concrete implementation whose Go type name would otherwise collide with the
// catch-all struct's own name (e.g. an interface "Content" with an
// implementation literally named "ContentOther"). Generation must succeed and
// the resulting Go code must compile, rather than failing with a misleading
// "genqlient internal error: conflicting definition" diagnostic.
func TestOmitImplsCatchAllNameCollision(t *testing.T) {
	tmpDir := t.TempDir()
	schemaPath := filepath.Join(tmpDir, "schema.graphql")
	queryPath := filepath.Join(tmpDir, "Repro.graphql")
...
type ContentOther implements Content {
  id: ID!
  flag: Boolean!
}
`
...
`
	...
	// The real concrete impl ContentOther must take the "Other" name; the
	// catch-all must fall back to "OtherCatchAll" rather than collide.
	out := string(generated["out.go"])
	if !strings.Contains(out, "type ReproRandomItemContentOther struct") {
		t.Errorf("expected concrete impl ReproRandomItemContentOther struct in generated code")
	}
	if !strings.Contains(out, "type ReproRandomItemContentOtherCatchAll struct") {
		t.Errorf("expected catch-all ReproRandomItemContentOtherCatchAll struct in generated code")
	}

	if !testing.Short() {
		if err := buildGoFile("OmitImplsCollision", generated["out.go"]); err != nil {
