...
type Config struct {
	...
	// set. A single catch-all struct is generated per interface selection to
	// carry the shared fields and preserve the previous fallback behavior
	// for any __typename returned by the server that we don't have a typed
	// representation for.
